...
export const expenses = pgTable("expenses", {
  ...
  invoiceId: integer("invoice_id").references(() => invoices.id), // Optional invoice association
  ...
  notes: text("notes"),
  autoGenerated: boolean("auto_generated").default(false), // Indicates if expense was auto-generated from invoice (true) or created manually (false)
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const customerActivities = pgTable("customer_activities", {
  ...
  activityStatus: integer("activity_status").notNull(), // 1: Active/Completed, 0: Inactive/Pending
  activityDate: timestamp("activity_date").defaultNow().notNull(),
  activityTableId: integer("activity_table_id"), // ID of related table (invoice_id, booking_id, etc.)
  activityTableName: text("activity_table_name"), // Name of related table (invoices, bookings, etc.)
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
