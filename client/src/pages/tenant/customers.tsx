      ...
      try {
        ...
        try {
          const response = await apiRequest(
            "GET",
            `/api/tenants/${tenant.id}/consulation-form`,
            {}
          );
...
function ConsulationFormDialog({
  ...
  // Import apiRequest for saving to database
  // Note: apiRequest is already imported at the top of the file
  ...
  useEffect(() => {
    setFormValues((prev) => {
      ...
      fields.forEach((field) => {
        nextValues[field.id] = prev[field.id] ?? "";
      });
      return nextValues;
    });
  }, [fields]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (fields.length === 0) {
      toast({
        ...
        description:
          "Please add at least one field before saving the consulation form.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (data?.success) {
        ...
        toast({
          title: "Consulation form saved",
          description:
            "Your consulation form layout has been saved successfully.",
        });
        onOpenChange(false);
      } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl">
        <DialogHeader>
          <DialogTitle>Set Consulation Form</DialogTitle>
          <DialogDescription>
            Build a custom consulation form with dynamic fields. These fields
